...
var emailNotVerified = function(pause) {
  if (!(Meteor.user() || Meteor.loggingIn())) {
    Router.go('login');
    pause();
  } else {
    for (var i = 0; i < Meteor.user().emails.length; i++) {
      if (Meteor.user().emails[i].verified !== 1) {
        Router.go('emailNotVerified', {email: Meteor.user().emails[i].address});
        pause();
      }
    }
  }
...
var organisationNotVerified = function(pause) {
  if (!(Meteor.user() || Meteor.loggingIn())) {
    Router.go('login');
    pause();
  } else {
    organisation = Meteor.user().profile.organisation;
    if (organisation.verified !== 1) {
...
Router.onBeforeAction(emailNotVerified, {except: includeNonUserRoutes(new Array())});
Router.onBeforeAction(organisationNotVerified, {except: includeNonUserRoutes(new Array())});
